...
| REQ-013 | Docker runtime supports both React UI and Python backend services | `Dockerfile`, `backend/Dockerfile`, `docker-compose.yml` | Manual container smoke check | `docker compose build`, `docker compose up -d`, `curl -I http://localhost:3000/`, `curl -s http://localhost:8000/health` | `README.md`, `docs/oracle-agentic-runtime.md` |
...
| REQ-020 | Repository preserves build, lint, and combined test automation | `package.json`, `tests/rendered-html.test.mjs`, `backend/tests/test_runtime.py` | `tests/rendered-html.test.mjs`, `backend/tests/test_runtime.py` | `pnpm run lint`, `pnpm run build`, `pnpm test` | `README.md` |
